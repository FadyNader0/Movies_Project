...
export default function GetGenre({ IDs }) {
  ...
  useEffect(() => {
    fetch('https://api.themoviedb.org/3/genre/movie/list', options)
      .then(res => res.json())
      .then(data => {
        const filteredGenres = data.genres.filter(genre => IDs.includes(genre.id));
        setGenresNames(filteredGenres.map(genre => genre.name)); 
      })
      .catch(err => {
        console.error(err);
      });
